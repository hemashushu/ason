...
mod tests {
    ...
    fn test_print_date() {
        ...
        assert_eq!(
            print_token_to_string(&Token::Date(d1)),
            "d\"2024-03-16T16:30:50+08:00\""
        );
        assert_eq!(
            print_token_to_string(&Token::Date(d2)),
    ...
    fn test_print_list() {
        ...
        let mut output = Vec::new();
        let mut writer = TokenStreamWriter::new(&mut output);
        writer.print_token(&Token::OpeningBracket).unwrap();
        print_tokens_with_space_separated_to_string(
            &mut writer,
            &[Token::Number(NumberToken::I32(1))],
        );
        writer.print_token(&Token::ClosingBracket).unwrap();
...
[
    ...
    fn test_print_named_list() {
        ...
        writer.print_token(&Token::OpeningBracket).unwrap();
        writer
            .print_token(&Token::String("foo".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Number(NumberToken::I32(123)))
            .unwrap();
        writer.print_token(&Token::ClosingBracket).unwrap();
...
[
    \"foo\": 123
        ...
        writer.print_space().unwrap();
        writer
            .print_token(&Token::String("bar".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::String("Alice".to_owned()))
            .unwrap();
        writer.print_token(&Token::ClosingBracket).unwrap();
...
[
    ...
    fn test_print_tuple() {
        ...
        let mut output = Vec::new();
        let mut writer = TokenStreamWriter::new(&mut output);
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        print_tokens_with_space_separated_to_string(
            &mut writer,
            &[
                Token::Number(NumberToken::I32(1)),
        ...
        writer.print_token(&Token::ClosingParenthesis).unwrap();

        assert_eq!(String::from_utf8(output).unwrap(), "(1 \"Alice\" true)");
    }
    ...
    fn test_print_object() {
        ...
        let mut output = Vec::new();
        let mut writer = TokenStreamWriter::new(&mut output);
        writer.print_token(&Token::OpeningBrace).unwrap();
        writer
            .print_token(&Token::Identifier("id".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Number(NumberToken::I32(123)))
            .unwrap();
        writer.print_token(&Token::ClosingBrace).unwrap();
            ...
            "\
{
    id: 123
        ...
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Identifier("name".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            ...
            "\
{
    id: 123 name: \"Alice\"
        ...
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Identifier("address".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer.print_token(&Token::OpeningBrace).unwrap();
        writer
            .print_token(&Token::Identifier("city".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::String("New York".to_owned()))
            .unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Identifier("zip".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer
            .print_token(&Token::String("10001".to_owned()))
            .unwrap();
        writer.print_token(&Token::ClosingBrace).unwrap();
        writer.print_token(&Token::ClosingBrace).unwrap();
            ...
            "\
{
    id: 123 address: {
        ...
        writer.print_space().unwrap();
        writer
            .print_token(&Token::Identifier("tags".to_owned()))
            .unwrap();
        writer.print_token(&Token::Colon).unwrap();
        writer.print_space().unwrap();
        writer.print_token(&Token::OpeningBracket).unwrap();
            ...
            "\
{
    ...
    fn test_print_variant_with_single_value() {
        ...
        let mut writer = TokenStreamWriter::new(&mut output);
        writer
            .print_token(&Token::Variant("Option".to_owned(), "Some".to_owned()))
            .unwrap();
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        writer
            .print_token(&Token::Number(NumberToken::I32(123)))
            .unwrap();
        writer.print_token(&Token::ClosingParenthesis).unwrap();
        assert_eq!(String::from_utf8(output).unwrap(), "Option::Some(123)");

        // test print variant with a string value, e.g., `Result::Err("error message")`
        let mut output = Vec::new();
        let mut writer = TokenStreamWriter::new(&mut output);
        writer
            .print_token(&Token::Variant("Result".to_owned(), "Err".to_owned()))
            .unwrap();
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        writer
            .print_token(&Token::String("Error message".to_owned()))
            .unwrap();
        writer.print_token(&Token::ClosingParenthesis).unwrap();
        assert_eq!(
            String::from_utf8(output).unwrap(),
        ...
        let mut writer = TokenStreamWriter::new(&mut output);
        writer
            .print_token(&Token::Variant("Variant".to_owned(), "List".to_owned()))
            .unwrap();
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        writer.print_token(&Token::OpeningBracket).unwrap();
        print_tokens_with_space_separated_to_string(
        ...
        let mut writer = TokenStreamWriter::new(&mut output);
        writer
            .print_token(&Token::Variant("Variant".to_owned(), "Object".to_owned()))
            .unwrap();
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        writer.print_token(&Token::OpeningBrace).unwrap();
        writer
    ...
    fn test_print_tuple_like_variant() {
        ...
        let mut writer = TokenStreamWriter::new(&mut output);
        writer
            .print_token(&Token::Variant("Variant".to_owned(), "Tuple".to_owned()))
            .unwrap();
        writer.print_token(&Token::OpeningParenthesis).unwrap();
        print_tokens_with_space_separated_to_string(
            &mut writer,
